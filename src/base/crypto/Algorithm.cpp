...
const char *Algorithm::kRX_TNFT         = "rx/tnft";
#endif
...
#   ifdef XMRIG_ALGO_RANDOMX
    ...
    ALGO_ALIAS_AUTO(RX_TNFT),       ALGO_ALIAS(RX_TNFT,         "randomx/tnft"),
                                    ALGO_ALIAS(RX_TNFT,         "randomtnft"),
...
std::vector<xmrig::Algorithm> xmrig::Algorithm::all(const std::function<bool(const Algorithm &algo)> &filter)
{
    static const std::vector<Id> order = {
        ...
        CN_UPX2,
        RX_0, RX_WOW, RX_ARQ, RX_GRAFT, RX_SFX, RX_KEVA, RX_TNFT,
